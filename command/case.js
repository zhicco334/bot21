		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Nih Kak Contact Owner Ku, Mereka Wibu🤝`, text, {quoted: hehe})
				}
			break
...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + `FN: Aprii\n`  
            + `ORG:Aprilia;\n` 
            + `TEL;type=CELL;type=VOICE;waid=616075336978:+62818301179\n`  
            + 'END:VCARD'  
...
case 'memegenerator': case 'memegen':{
						...
						if (args.length < 1) return reply(`Kirim perintah *.${command}* Zhiccobot`)
									if (q.includes('|')) return reply(`Kirim perintah *.${command}* Zhiccobot`)
									try {
										if (!isQuotedImage) return reply(`Reply Gambar!`)
										reply(lang.wait())
										var teks2 = args.join(' ')
